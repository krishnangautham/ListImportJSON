/**
 * Imports a list of JSON feed and returns the results to be inserted into a Google Spreadsheet. The JSON feed is flattened to create
 * a two-dimensional array. For every JSON feed in the list, the first row contains the headers, with each column header indicating the path to that data in
 ...
 *
 *   ==ListImportJSON("ebiSearchList",51,100,6)
 *
 ...
 **/
function ListImportJSON(sourceSheet, fromRow, toRow, queryColumnNumber, query, parseOptions) {
    fromRow = fromRow -1;
    toRow = toRow-1;
    var sheet = SpreadsheetApp.getActive().getSheetByName(sourceSheet);
    var data = sheet.getDataRange().getValues();
    var output = [];
    for (var i = fromRow; i <= toRow; i++) {
        var obj = ImportJSON(data[i][queryColumnNumber], query, parseOptions);
